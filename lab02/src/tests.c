...
struct image generate_rand_img() {
    ...
    if (img.px == NULL) assert(0 && "Rerun test, malloc failed");
...
struct image duplicate_img(struct image img) {
    ...
    if (img_dup.px == NULL) assert(0 && "Rerun test, malloc failed");
    for (long i = 0; i < img.size_y * img.size_x; i++) {
...
START_TEST(grayscale_functionality) {
    ...
    for (long i = 0; i < sz_y; i++)
        for (long j = 0; j < sz_x; j++) img.px[i * sz_x + j].alpha = rand_alpha;
...
char *sepia_summers[] = {
    "test_imgs/summer_sepia_0.png", "test_imgs/summer_sepia_10.png",
    "test_imgs/summer_sepia_20.png", "test_imgs/summer_sepia_30.png",
    "test_imgs/summer_sepia_42.png"};
uint8_t sepia_depths[] = {0x0, 0x10, 0x20, 0x30, 0x42};
...
uint8_t bw_thresholds[] = {0x40, 0x80};
...
START_TEST(grayscale_examples) {
    ...
    ck_assert_uint_eq(img_out->size_x, img->size_x);
    ck_assert_uint_eq(img_out->size_y, img->size_y);
    for (long j = 0; j < img->size_x * img->size_y; j++) {
        ck_assert_uint_eq(img_out->px[j].red, img->px[j].red);
        ck_assert_uint_eq(img_out->px[j].green, img->px[j].green);
        ck_assert_uint_eq(img_out->px[j].blue, img->px[j].blue);
        ck_assert_uint_eq(img_out->px[j].alpha, img->px[j].alpha);
    }
    free(img_out->px);
    free(img->px);
    free(img_out);
...
START_TEST(blur_functionality) {
    ...
    struct image img_dup = duplicate_img(img);
    int radius = 0;
    filter_blur(&img_dup, &radius);
    for (int i = 0; i < img_dup.size_y * img_dup.size_x; i++) {
        ck_assert_uint_eq(img_dup.px[i].red, img.px[i].red);
        ck_assert_uint_eq(img_dup.px[i].green, img.px[i].green);
        ck_assert_uint_eq(img_dup.px[i].blue, img.px[i].blue);
        ck_assert_uint_eq(img_dup.px[i].alpha, img.px[i].alpha);
    }
    free(img_dup.px);
    ...
    img_dup = duplicate_img(img);
    radius = 1;
    struct pixel px_check[3][3] = {
        {dark2, dark1, dark2}, {dark1, dark0, dark1}, {dark2, dark1, dark2}};
    struct image img_check = {3, 3, &px_check};
    filter_blur(&img_dup, &radius);
    for (int i = 0; i < img_dup.size_y * img_dup.size_x; i++) {
        ck_assert_uint_eq(img_dup.px[i].red, img_check.px[i].red);
        ck_assert_uint_eq(img_dup.px[i].green, img_check.px[i].green);
        ck_assert_uint_eq(img_dup.px[i].blue, img_check.px[i].blue);
        ck_assert_uint_eq(img_dup.px[i].alpha, img_check.px[i].alpha);
    }
    free(img_dup.px);
    // radius 2 and 3
    for (int i = 2; i < 4; i++) {
        ...
        for (int j = 0; j < img_dup.size_y * img_dup.size_x; j++) {
            ck_assert_uint_eq(img_dup.px[j].red, dark0.red);
            ck_assert_uint_eq(img_dup.px[j].green, dark0.green);
            ck_assert_uint_eq(img_dup.px[j].blue, dark0.blue);
            ck_assert_uint_eq(img_dup.px[j].alpha, dark0.alpha);
        }
        free(img_dup.px);
    }
    /* TODO: Implement */
...
START_TEST(blur_radius_edge_cases) {
    ...
    struct image img = generate_rand_img();
    int radii[4][5] = {
        {INT_MIN, INT_MAX, 0, img.size_x, img.size_y},
        {INT_MIN / 2, INT_MAX / 2, 0, img.size_x / 2, img.size_y / 2},
        {INT_MIN + 1, INT_MAX + 1, 1, img.size_x + 1, img.size_y + 1},
        {INT_MIN - 1, INT_MAX - 1, -1, img.size_x - 1, img.size_y - 1}};
    for (int i = 0; i < 20; i++) {
        struct image img_dup = duplicate_img(img);
        filter_blur(&img_dup, &radii[i]);
        free(img_dup.px);
    }
}
END_TEST
...
START_TEST(transparency_functionality) {
    ...
    for (int j = 0; j < img.size_y * img.size_x; j++) {
        ck_assert_uint_eq(img.px[j].alpha, alpha);
    }
}
END_TEST

/* Check if the function crashes when we pass nullptr as the argument */
START_TEST(transparency_edge_case) { filter_transparency(NULL, NULL); }
END_TEST
...
int main() {
    ...
    tcase_add_loop_test(tc2, sepia_example_image, 0,
                        sizeof(sepia_depths) / sizeof(sepia_depths[0]));
    tcase_add_loop_test(tc2, bw_example_image, 0,
                        sizeof(bw_summers) / sizeof(bw_summers[0]));
